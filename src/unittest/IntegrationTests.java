...
public class IntegrationTests {
	...
	public void test() throws Exception
	{
		...
		for(int j=0;j<3;j++)
		{
			for(int i=0;i<3;i++)
			{
				if(sp.findIntersections(cam.constructRayThroughPixel(3, 3, j, i))!=null)
				 mone+=sp.findIntersections(cam.constructRayThroughPixel(3, 3, j, i)).size();
			}
		}
...
//TC04:Fourth test case(9)
	...
	for(int j=0;j<3;j++)
	{
		for(int i=0;i<3;i++)
		{
			if(pl.findIntersections(cam.constructRayThroughPixel(3, 3, j, i))!=null)
			 mone+=pl.findIntersections(cam.constructRayThroughPixel(3, 3, j, i)).size();
		}
	}
	...
	for(int j=0;j<3;j++)
	{
		for(int i=0;i<3;i++)
		{
			if(tr.findIntersections(cam.constructRayThroughPixel(3, 3, j, i))!=null)
			 mone+=tr.findIntersections(cam.constructRayThroughPixel(3, 3, j, i)).size();
		}
	}
